...
//  tic-tac-toe
//
//  Created by Kristina Gelzinyte on 11/9/17.
        ...
        if state == .winner{
            ...
            DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(delay) * Int64(NSEC_PER_SEC)) / Double(NSEC_PER_SEC)) {
                    ...
                    let gridNode = self.scene?.childNode(withName: "grid")
                    let cellNode = gridNode?.childNode(withName: self.scene!.gameBoard!.getElementAtBoardLocation(move!.cell).node)
